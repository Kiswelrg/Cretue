...
function move(id){
    ...
    if(opt == 0){
        ...
        scrollto(blocks.indexOf(0)-id,2,false);
        $(drt).animate({ 
            left: r(mv,id),
        }, c );
        $(drt).animate({
...
function doSetTimeout(i) {
    if(opt==0){
        setTimeout(function() { move(plain[i]);
            }, (i-n_size*n_size*2-2-(parseInt(n_size/4)*6))*160 );//(i-n_size*n_size*3+(n_size/4*6))*120
    }
    else if(opt==1){
        setTimeout(function() { move(b_actions[i]-blocks.indexOf(0));
            }, c*3*i);
    }
